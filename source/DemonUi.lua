...
local T = {
    ...
    AccentGlow  = Color3.fromRGB(60, 110, 220),
...
local function Corner(r)  return New("UICorner",{CornerRadius=r or UDim.new(0,7)}) end
...
local function Pad(t,r,b,l) return New("UIPadding",{PaddingTop=UDim.new(0,t or 6),PaddingRight=UDim.new(0,r or 8),PaddingBottom=UDim.new(0,b or 6),PaddingLeft=UDim.new(0,l or 8)}) end
local function List(dir,gap) return New("UIListLayout",{FillDirection=dir or Enum.FillDirection.Vertical,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,gap or 4)}) end

local function Tw(inst,props,t,sty,dir)
    local tw = TweenService:Create(inst,TweenInfo.new(t or 0.22,sty or Enum.EasingStyle.Quart,dir or Enum.EasingDirection.Out),props)
    tw:Play(); return tw
end

-- Fix: draggable yang TIDAK konflik dengan slider / input lain
local function MakeDraggable(frame, handle)
    handle = handle or frame
    local drag, ds, sp = false, nil, nil
    handle.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            -- hanya drag jika target adalah handle itu sendiri (bukan child interaktif)
...
local function EnsureNotif()
    ...
    NotifSG = New("ScreenGui",{Name="DemonNotifs",ResetOnSpawn=false,ZIndexBehavior=Enum.ZIndexBehavior.Sibling,Parent=LocalPlayer.PlayerGui})
    NotifHolder = New("Frame",{BackgroundTransparency=1,Size=UDim2.new(0,300,1,0),Position=UDim2.new(1,-308,0,0),Parent=NotifSG},{List(Enum.FillDirection.Vertical,6)})
...
function DemonUI:Notify(opts)
    EnsureNotif()
    opts = opts or {}
    local accent = ({info=T.Accent,success=T.Success,warning=T.Warning,danger=T.Danger})[opts.Type or "info"] or T.Accent

    local card = New("Frame",{BackgroundColor3=T.Item,Size=UDim2.new(1,-8,0,66),BackgroundTransparency=1,Parent=NotifHolder},{Corner(UDim.new(0,8)),Stroke(accent,1)})
...
local function ShowWelcome(sg, onDone)
    ...
    local function Orb(x,y,size,color)
        ...
        -- blur effect via stacked frames
        for i=1,3 do
            New("Frame",{
                BackgroundColor3=color,
    ...
    task.spawn(function()
        ...
        -- fade in title char by char
        local fullTitle = "Demon UI"
...
function DemonUI:CreateWindow(opts)
    opts = opts or {}
    local W_SIZE    = opts.Size        or UDim2.fromOffset(590, 465)
    local TITLE     = opts.Title       or "Demon UI"
    local SUBTITLE  = opts.SubTitle    or "Made By Jova"
    local MIN_KEY   = opts.MinimizeKey or Enum.KeyCode.RightControl

    -- apply theme overrides
    for k,v in pairs(opts.Theme or {}) do if T[k]~=nil then T[k]=v end end

    local old = LocalPlayer.PlayerGui:FindFirstChild("DemonUI_Main")
    if old then old:Destroy() end

    local SG = New("ScreenGui",{Name="DemonUI_Main",ResetOnSpawn=false,ZIndexBehavior=Enum.ZIndexBehavior.Sibling,Parent=LocalPlayer.PlayerGui})

    -- show welcome first, then reveal window
    ...
    local MF = New("Frame",{
        ...
        Size=W_SIZE,
        Position=UDim2.new(0.5,-W_SIZE.X.Offset/2,0.5,-W_SIZE.Y.Offset/2),
        BackgroundTransparency=1,
        Parent=SG
    },{Corner(UDim.new(0,10)),Stroke(T.Border)})

    MakeDraggable(MF)   -- window level drag (akan di-override per handle)

    -- ── TopBar ──
    local TB = New("Frame",{BackgroundColor3=T.TopBar,Size=UDim2.new(1,0,0,40),Parent=MF},{Corner(UDim.new(0,10)),Stroke(T.Border)})
    -- fill bottom gap of topbar corners
    New("Frame",{BackgroundColor3=T.TopBar,Size=UDim2.new(1,0,0,10),Position=UDim2.new(0,0,1,-10),BorderSizePixel=0,Parent=TB})

    -- title
    New("TextLabel",{Text=TITLE.."  ·  "..SUBTITLE,Font=T.FontBold,TextSize=13,TextColor3=T.TextPri,BackgroundTransparency=1,Size=UDim2.new(1,-90,1,0),Position=UDim2.new(0,14,0,0),TextXAlignment=Enum.TextXAlignment.Left,Parent=TB})

    -- window buttons
    local function WinBtn(pos,bg,tc,txt)
        local b = New("TextButton",{Text=txt,Font=T.Font,TextSize=12,TextColor3=tc,BackgroundColor3=bg,Size=UDim2.new(0,26,0,26),Position=pos,AutoButtonColor=false,Parent=TB},{Corner(UDim.new(0,5))})
    ...
    -- drag via topbar only (not child buttons)
    MakeDraggable(MF, TB)
    ...
    MinBtn.MouseButton1Click:Connect(function()
        ...
        Tw(MF,{Size= minimized and UDim2.new(0,W_SIZE.X.Offset,0,40) or W_SIZE},0.28,Enum.EasingStyle.Quart)
    end)
    CloseBtn.MouseButton1Click:Connect(function()
        Tw(MF,{Size=UDim2.new(0,W_SIZE.X.Offset,0,0),BackgroundTransparency=1},0.22)
        task.wait(0.25); SG:Destroy()
    end)
    UIS.InputBegan:Connect(function(inp,gpe)
        if gpe then return end
        if inp.KeyCode == MIN_KEY then
            minimized = not minimized
            Body.Visible = not minimized
            Tw(MF,{Size= minimized and UDim2.new(0,W_SIZE.X.Offset,0,40) or W_SIZE},0.28,Enum.EasingStyle.Quart)
        end
    ...
    -- ── AddTab ──
    function Win:AddTab(opts2)
        opts2 = opts2 or {}
        local name = opts2.Title or "Tab"
        ...
        -- ── SECTION ──
        function Tab:AddSection(name2)
            New("TextLabel",{Text=string.upper(name2),Font=T.FontBold,TextSize=10,TextColor3=T.TextDim,BackgroundTransparency=1,Size=UDim2.new(1,0,0,22),TextXAlignment=Enum.TextXAlignment.Left,Parent=Page},{Pad(10,0,0,4)})
        end
        ...
        function Tab:AddParagraph(o)
            ...
            if o.Title then New("TextLabel",{Text=o.Title,Font=T.FontBold,TextSize=13,TextColor3=T.TextPri,BackgroundTransparency=1,Size=UDim2.new(1,0,0,18),TextXAlignment=Enum.TextXAlignment.Left,Parent=f}) end
            if o.Content then
        ...
        function Tab:AddToggle(id,o)
            ...
            _G["DemonToggle_"..(id or o.Title or "")]=obj
            return obj
        end

        -- ── SLIDER ──
        -- FIX: slider input handler HANYA pada slider itu sendiri,
        --      bukan lewat UserInputService global yang menyebabkan window ikut gerak
        function Tab:AddSlider(id,o)
            ...
            local rowH=(o.Description and o.Description~="") and 64 or 50
            ...
            -- invisible hit area (lebih tinggi agar mudah klik)
            local hit=New("TextButton",{Text="",BackgroundTransparency=1,Size=UDim2.new(1,0,0,22),Position=UDim2.new(0,0,0.5,-11),ZIndex=5,Parent=track,AutoButtonColor=false})
            ...
            local function Calc(absX)
                ...
                local rounded=math.round(raw/rnd)*rnd
                val=rounded; obj.Value=val
            ...
            -- hanya mulai drag dari hit area slider (bukan window)
            hit.InputBegan:Connect(function(inp)
                if inp.UserInputType==Enum.UserInputType.MouseButton1 then
                    isDragging=true
                    Tw(thumb,{Size=UDim2.new(0,15,0,15),Position=UDim2.new(thumb.Position.X.Scale,-7.5,0.5,-7.5)},0.1,Enum.EasingStyle.Back)
                    Calc(inp.Position.X)
                end
            end)
            -- InputChanged di hit area (tidak bocor ke window)
            hit.InputChanged:Connect(function(inp)
                if isDragging and inp.UserInputType==Enum.UserInputType.MouseMovement then
                    Calc(inp.Position.X)
                end
            end)
            -- Global InputChanged hanya untuk movement saat drag (tidak picu window drag)
            local conn
            hit.InputBegan:Connect(function(inp)
                if inp.UserInputType==Enum.UserInputType.MouseButton1 then
            ...
            _G["DemonSlider_"..(id or o.Title or "")]=obj
            return obj
        end
        ...
        function Tab:AddDropdown(id,o)
            ...
            local cur=o.Default or (vals[1] or ""); local sel={}
            local open=false
            ...
            local function Build()
                ...
                for _,v in ipairs(vals) do
                    ...
                    itm.MouseButton1Click:Connect(function()
                        if multi then local idx=table.find(sel,v); if idx then table.remove(sel,idx) else table.insert(sel,v) end; obj.Value=sel; dBtn.Text=#sel>0 and table.concat(sel,", ") or "None"
                        else cur=v; obj.Value=v; dBtn.Text=v; Close() end
                        if o.Callback then o.Callback(obj.Value) end; Build()
                    end)
                end
                LS.CanvasSize=UDim2.new(0,0,0,IL.AbsoluteContentSize.Y+8)
                local h=math.min(#vals*31+8,160)
                Tw(LF,{Size=UDim2.new(0,116,0,h)},0.2,Enum.EasingStyle.Back)
            end
            ...
            _G["DemonDropdown_"..(id or o.Title or "")]=obj
            return obj
        end
        ...
        function Tab:AddInput(id,o)
            ...
            _G["DemonInput_"..(id or o.Title or "")]=obj; return obj
        end

        -- ── KEYBIND ──
        function Tab:AddKeybind(id,o)
            o=o or {}
            local cur=o.Default or Enum.KeyCode.Unknown; local listening=false
            ...
            UIS.InputBegan:Connect(function(inp,gpe)
                if listening and inp.UserInputType==Enum.UserInputType.Keyboard then
                    ...
                    Tw(kBtn,{BackgroundColor3=T.SliderTrack},0.14); if o.Callback then o.Callback(cur) end
                end
                if not gpe and not listening and inp.KeyCode==cur then if o.OnPress then o.OnPress() end end
            end)
            function obj:Set(k) cur=k; obj.Value=k; kBtn.Text=k.Name end
            _G["DemonKeybind_"..(id or o.Title or "")]=obj; return obj
        end
        ...
        function Tab:AddColorPicker(id,o)
            ...
            hBar.InputBegan:Connect(function(inp)
                if inp.UserInputType==Enum.UserInputType.MouseButton1 then
                    hDrag=true; local p=math.clamp((inp.Position.X-hBar.AbsolutePosition.X)/hBar.AbsoluteSize.X,0,1)
            ...
            hexB.FocusLost:Connect(function()
                local h=hexB.Text:gsub("#",""); if #h==6 then
            ...
            prev.MouseButton1Click:Connect(function()
                ...
                else Tw(pF,{Size=UDim2.new(0,224,0,0)},0.18); task.wait(0.2); pF.Visible=false end
            end)
            UIS.InputBegan:Connect(function(inp)
            ...
            _G["DemonColor_"..(id or o.Title or "")]=obj; return obj
        end
    ...
    function Win:SelectTab(i) if self._tabs[i] then self._tabs[i]._select() end end

    -- ── Run welcome screen, then reveal window ──
    ShowWelcome(SG, function()
        -- animate window in
        MF.Size = UDim2.new(0,W_SIZE.X.Offset,0,0)
        Tw(MF,{BackgroundTransparency=0},0.1)
        Tw(MF,{Size=W_SIZE},0.38,Enum.EasingStyle.Back)
    end)
...
function DemonUI:SetTheme(opts)
    for k,v in pairs(opts or {}) do if T[k]~=nil then T[k]=v end end
end
